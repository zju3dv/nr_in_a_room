...
def bash_run(cmd):
    ...
    # colmap_bin = "/home/ybbbbt/Developer/colmap/build/src/exe/colmap"
    colmap_bin = "colmap"
    cmd = colmap_bin + " " + cmd
    print("\nRunning cmd: ", cmd)
...
def run_sift_matching(img_dir, db_file, remove_exist=False):
    ...
    cmd = " {} --database_path {} \
        ...
        matcher_type, db_file, gpu_index
    )
...
def run_sfm(img_dir, db_file, out_dir):
    ...
    cmd = " mapper \
            ...
            --Mapper.filter_min_tri_angle 3.0".format(
        db_file, img_dir, out_dir
    )
...
def prepare_mvs(img_dir, sparse_dir, mvs_dir):
    ...
    cmd = " image_undistorter \
            ...
            --output_path {} \
            --output_type COLMAP \
            --max_image_size 2000".format(
        img_dir, sparse_dir, mvs_dir
    )
...
def run_photometric_mvs(mvs_dir, window_radius):
    ...
    cmd = " patch_match_stereo --workspace_path {} \
                    ...
                    --PatchMatchStereo.num_iterations 12".format(
        mvs_dir, window_radius, gpu_index
    )
...
def main(img_dir, out_dir, run_mvs=False):
    ...
    normalize_cam_dict(
        os.path.join(out_dir, "posed_images/nerfpp_cameras.json"),
        os.path.join(out_dir, "posed_images/nerfpp_cameras_normalized.json"),
    )
...
if __name__ == "__main__":
    ...
    # img_dir = "/home/ybbbbt/Developer/neural_scene/data/arkit_recon/arkit_desk_2_colmap_scratch/images"
    # out_dir = "/home/ybbbbt/Developer/neural_scene/data/arkit_recon/arkit_desk_2_colmap_scratch/output"
    img_dir = sys.argv[1]
    out_dir = sys.argv[2]
