...
def run_point_triangulation(img_dir, db_file, out_dir):
    ...
    cmd = " point_triangulator  --database_path {} \
        ...
        db_file, img_dir, out_dir, out_dir
    )
    bash_run(cmd)
...
def run_global_ba(in_dir, out_dir):
    ...
    cmd = " bundle_adjuster --input_path {in_dir} --output_path {out_dir}".format(
...
def prepare_mvs(img_dir, sfm_dir, mvs_dir):
    ...
    os.symlink(os.path.relpath(img_dir, mvs_dir), images_symlink)

    sparse_symlink = os.path.join(mvs_dir, "sparse/0/")
...
def main(img_dir, pinhole_dict_file, out_dir, run_mvs=False, refine_with_gba=True):
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    ...
    extract_all_to_dir(
        os.path.join(out_dir, "sfm"), os.path.join(out_dir, "nerfpp_fmt")
    )
...
def convert_cam_dict_to_pinhole_dict(cam_dict_file, pinhole_dict_file, img_dir):
    ...
    for img_name in cam_dict:
        ...
        # W2C : SLAM Twc with xyz-> right down forwawrd
...
if __name__ == "__main__":
    ...
    main(img_dir, pinhole_dict_file, out_dir, run_mvs)
