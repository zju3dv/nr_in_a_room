...
def parse_tracks(colmap_images, colmap_points3D):
    ...
    view_keypoints = (
        {}
    )  # dict of lists; each list represents the triangulated key points of a view
    ...
    for point3D_id in colmap_points3D:
        ...
        for i in range(cur_track_len):
            ...
            if img_name not in view_keypoints:
                view_keypoints[img_name] = [
                    (point2D[0], point2D[1]) + cur_track["xyz"] + (cur_track_len,),
                ]
            else:
                view_keypoints[img_name].append(
                    (point2D[0], point2D[1]) + cur_track["xyz"] + (cur_track_len,)
                )

        cur_track["pixels"] = sorted(
            pixels, key=lambda x: x[0]
...
def parse_camera_dict(colmap_cameras, colmap_images):
    ...
    for image_id in colmap_images:
        ...
        # W2C: SLAM Tcw, from world to camera
...
def extract_all_to_dir(sparse_dir, out_dir, ext=".bin"):
    ...
    camera_dict_file = os.path.join(out_dir, "nerfpp_cameras.json")
    xyz_file = os.path.join(out_dir, "nerfpp_points.txt")
    track_file = os.path.join(out_dir, "nerfpp_tracks.json")
    keypoints_file = os.path.join(out_dir, "nerfpp_keypoints.json")
    ...
    mesh = trimesh.Trimesh(
        vertices=all_points[:, :3].astype(np.float32),
        vertex_colors=all_points[:, -3:].astype(np.uint8),
    )
    mesh.export(os.path.join(out_dir, "nerfpp_points.ply"))
