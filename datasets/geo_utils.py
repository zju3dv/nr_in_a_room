...
def read_instance_points_from_file(
    mesh_ply_path, instance_indices_path, instance_id, num_points=5000
):
    scene_mesh = o3d.io.read_triangle_mesh(mesh_ply_path)
    instance_ids = np.load(instance_indices_path)
    assert len(instance_ids) == np.asarray(scene_mesh.vertices).shape[0]
    # remove other triangles
    rm_idx = np.arange(len(instance_ids))[instance_ids != instance_id]
    scene_mesh.remove_vertices_by_index(rm_idx)
    # sample sufficient points for current instance
    instance_pts = scene_mesh.sample_points_poisson_disk(
        number_of_points=num_points, init_factor=5
    )
    instance_pts = np.asarray(instance_pts.points)
    return instance_pts
