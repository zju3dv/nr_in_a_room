...
class RadianceNet(nn.Module):
    def __init__(
        ...
        self.input_light_ch = input_light_ch
        self.input_appearance_ch = input_appearance_ch
        ...
        if use_view_dirs:
            ...
            print("Radiance net not using view dir.")
            in_dim_0 = input_ch_pts + W_geo_feat + input_light_ch + input_appearance_ch
    ...
    def forward(
        ...
        if self.input_light_ch > 0:
            x = torch.cat([x, light_code], -1)

        if self.input_appearance_ch > 0:
            x = torch.cat([x, appearance_code], -1)
