...
if __name__ == "__main__":
    ...
    neus, render_kwargs_train, render_kwargs_test = get_model(
        ...
        config_path="config/neus_real_room_view_ind.yaml",
        # config_path="config/neus_no_radiance_pe.yaml",
        need_trainer=False,
        extra_conf=conf,
    )
    ...
    mesh.vertices = o3d.utility.Vector3dVector(
        vertices_.astype(np.float64) * args.scale_factor
    )
    ...
    with torch.no_grad():
        ...
        out_color_chunks = []
        for i in tqdm(range(0, B, args.chunk)):
            xyz_chunk = vertices[i : i + args.chunk]  # (N, 3)
            rays_d_chunk = rays_d[i : i + args.chunk].cuda()
            N_local_rays = xyz_chunk.shape[0]
            inst_embedded = image_attributes.embedding_instance(
                torch.ones((N_local_rays)).long().cuda() * obj_id
            )
            light_embedded = image_attributes.embedding_light(
                torch.zeros((N_local_rays)).long().cuda()
            )
            if args.use_emb_a:
                appearance_emb = image_attributes.embedding_light(
                    torch.ones((N_local_rays)).long().cuda()
                )
            else:
                appearance_emb = None
            res_chunk = neus.forward(
                x=xyz_chunk,
                light_code=light_embedded,
                view_dirs=rays_d_chunk,
                obj_code=inst_embedded,
                appearance_code=appearance_emb,
            )[0]
    ...
    write_json(
        ...
        os.path.join(f"debug/bbox_{obj_id}.json"),
    )
    print(bbox)
